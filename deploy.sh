...
interactive_config() {
    ...
    echo
    echo -e "${GREEN}LLM Configuration${NC}"
    echo "Available providers: auto, ollama, openai, openrouter, fallback"
    echo "  auto       - Try services in order (Ollama → OpenAI → OpenRouter → Fallback)"
    echo "  ollama     - Local Ollama only"
    echo "  openai     - OpenAI GPT-4o only"
    echo "  openrouter - OpenRouter Claude 3 only"
    echo "  fallback   - Basic reports, no LLM"
    echo
    read -p "LLM Provider [$LLM_PROVIDER]: " input
    LLM_PROVIDER="${input:-$LLM_PROVIDER}"
    
    if [[ "$LLM_PROVIDER" == "ollama" ]] || [[ "$LLM_PROVIDER" == "auto" ]]; then
        read -p "Ollama URL [$OLLAMA_URL]: " input
        OLLAMA_URL="${input:-$OLLAMA_URL}"
    fi
    
    if [[ "$LLM_PROVIDER" == "openai" ]] || [[ "$LLM_PROVIDER" == "auto" ]]; then
        if [ -z "$OPENAI_API_KEY" ]; then
            read -p "OpenAI API Key (leave blank to skip): " OPENAI_API_KEY
        else
            read -p "OpenAI API Key [****configured****]: " input
            OPENAI_API_KEY="${input:-$OPENAI_API_KEY}"
        fi
    fi
    
    if [[ "$LLM_PROVIDER" == "openrouter" ]] || [[ "$LLM_PROVIDER" == "auto" ]]; then
        if [ -z "$OPENROUTER_API_KEY" ]; then
            read -p "OpenRouter API Key (leave blank to skip): " OPENROUTER_API_KEY
        else
            read -p "OpenRouter API Key [****configured****]: " input
...
EOF
    ...
    if [[ "${ADMIN_PASSWORD:-admin123}" == "admin123" ]]; then
        log_warning "Using default admin password - change ADMIN_PASSWORD for production!"
    fi
}
...
show_help() {
    ...
    echo
    echo -e "${GREEN}LLM Environment Variables:${NC}"
    echo "  LLM_PROVIDER      auto|ollama|openai|openrouter|fallback (default: auto)"
    echo "  OLLAMA_URL        Ollama server URL (default: http://localhost:11434)"
    echo "  OPENAI_API_KEY    OpenAI API key"
    echo "  OPENROUTER_API_KEY OpenRouter API key"
    echo "  ADMIN_PASSWORD    Admin panel password (default: admin123)"
    echo "  DOCTOR_PASSWORD   Doctor panel password (default: doctor123)"
    echo
    ...
    echo "  sudo LLM_PROVIDER=ollama ./deploy.sh deploy"
    echo "  sudo OPENAI_API_KEY=sk-xxx ./deploy.sh deploy"
    echo
}
